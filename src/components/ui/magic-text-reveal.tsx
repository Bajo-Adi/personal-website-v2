...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  // Use passed scroll progress or create own
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start end", "end start"]
  });

  // Create opacity transform for this single line - particles visible first, then text
  const lineOpacity = useTransform(scrollProgress || scrollYProgress, [0, 1], [0, 1]);
  const particleOpacity = useTransform(scrollProgress || scrollYProgress, [0, 1], [1, 0]);
  ...
  const updateParticles = useCallback((
    ...
    particles.forEach(particle => {
      if (showText) {
        // When text should show, gradually return to original position
        ...
        // Fade out particles when text should show
  ...
  }, [showText, spread, speed, globalDpr, updateParticles, renderParticles]);
